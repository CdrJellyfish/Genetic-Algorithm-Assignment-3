...
//System
    ...
    public void genNewPopulation(int half){
        ...
        if(bestTree == null){
         ...
         if(half == 1){
                //population.computeGlobalSimularity();
            }else{
                //population.computeLocalSimularity();
            }
    ...
    public void train(){
        ...
        for(int i = 0; i < epochs;i++){
            ...
            //  for (Tree tree : population.trees) {
            //     if(tree.isSameAs(tree.getRoot().getLeft(),tree.getRoot().getRight()));{
            //         tree.avgFitness = tree.avgFitness/2;
            //     }
            // }
    ...
    public void test(){
            ...
            for(int j = testStart; j < reader.dataAmount; j++){
                //System.out.println("targets : " +reader.target[j]);
                bestTree.updateValues(reader.data[j],reader.target[j]);
               
                bestTree.calculateAccuracy();
                // System.out.println("Calc fitness : " + bestTree.avgFitness+ "  AvgFit : " + bestTree.avgFitness );
