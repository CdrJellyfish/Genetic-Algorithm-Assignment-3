...
//System
    ...
    public boolean isSameAs(Node left, Node right) {
    ...
    // Check node type, variable name, value, and operator
    boolean sameType = a.getType() == b.getType();
    boolean sameVar = a.getvarName().equals(b.getvarName());
    boolean sameValue = Double.compare(a.getValue(), b.getValue()) == 0;
    boolean sameOp = a.getOperator().equals(b.getOperator());

    if (!sameType || !sameVar || !sameValue || !sameOp) return false;

    // Recursively check left and right children
    return areNodesEqual(a.getLeft(), b.getLeft()) && areNodesEqual(a.getRight(), b.getRight());
}

    public double calculateFitness(){
        if(getDepth() > overallMaxDepth+2 || getDepth() < overallMaxDepth-2){
            this.score = 0;
            this.fitness = -20;
        }else{
        ...
        if(target == this.score && 0<this.score){
        ...
        }else if(target == 1 && 0<this.score){
             this.fitness = -2;
        }
        else if(0<this.score){
            this.fitness = 0;
